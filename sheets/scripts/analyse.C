...
//    if (name.find("New")==std::string::npos) sprintf(hname, "%s-I", hname);
    ...
    canv->SaveAs((nameOfFile+"_uscms.png").c_str());
    canv->SaveAs((nameOfFile+"_uscms.C").c_str());
...
void analyse()
{
    ...
    std::cout << "wrong entries " << wrong << std::endl;
    //   return;
    TFile* rootfile = new TFile("analysis.root","RECREATE");
    ...
    for (int j = 0; j < 4; ++j) {
        ...
        fracUS[j]          ->SetFillColor(38);
        fracLPC[j]         ->SetFillColor(38);
        fracNonLPC[j]      ->SetFillColor(38);
        arcFracUS[j]       ->SetFillColor(38);
        arcFracLPC[j]      ->SetFillColor(38);
        arcFracNonLPC[j]   ->SetFillColor(38);
        arcFracLPCnew[j]   ->SetFillColor(38);
        arcFracNonLPCnew[j]->SetFillColor(38);
        usVsLpcFrac[j]     ->SetFillColor(46);
        usVsLpcNbr[j]      ->SetFillColor(46);
        fracLPCnew[j]      ->SetFillColor(38);
        fracNonLPCnew[j]   ->SetFillColor(38);
        usVsLPCnewFrac[j]  ->SetFillColor(46);
        usVsLPCnewNbr[j]   ->SetFillColor(46);
    }
